function [RMSE] = calibrate_gpp(params)
...
% Call GPP_dahra function to generate simulated GPP with and without scalar
[gpp_scalar] = GPP_dahra(params);

% RMSE Calculation
N = length(gpp_obs); % Total lenght of the data of interest
N2 = ceil(N/2); % First half of the data will be used for calibration
meanSum = 0;
...
% %mse = meanSum/N;
% 
% RMSE = sqrt(meanSum/N); % RMSE before calibration

% RMSE = sqrt( sum( (gpp_obs(:)-gpp_scalar(:)).^2) / numel(gpp_obs) );
